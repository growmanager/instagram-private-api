...
const ROUTES = {
    ...
    storyFeed: 'feed/user/<%= id %>/story/',
    storySeen_v2: 'media/seen/?reel=1&live_vod=0/',
    tagFeed: 'feed/tag/<%= encodeURI(tag) %>/?<%= maxId ? ("max_id=" + maxId + "&") : "" %>rank_token=<%= rankToken %>',
    selfLikedFeed: 'feed/liked/<%= maxId ? ("?max_id=" + maxId) : "" %>',
    locationFeed: 'locations/<%= id %>/sections/',
    locationFeedOld: 'feed/location/<%= id %>/?<%= maxId ? ("max_id=" + maxId + "&") : "" %>rank_token=<%= rankToken %>',
    ...
    mediaComments: 'media/<%= mediaId %>/comments/<%= maxId ? ("?max_id=" + encodeURIComponent(maxId)) : "" %>',
